...
var app = (function () {
    ...
    const getRoverDirection = (orientation) => {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			div = element("div");
    			img = element("img");
    			attr_dev(img, "style", img_style_value = `transform: rotate(${/*orientation*/ ctx[3]}deg)`);
    			attr_dev(img, "class", img_class_value = `ml-3 mt-3`);
    			if (img.src !== (img_src_value = Tiles.rover)) attr_dev(img, "src", img_src_value);
    			attr_dev(img, "title", img_title_value = `${/*title*/ ctx[0]}-${/*row*/ ctx[1]}-${/*col*/ ctx[2]}-${getRoverDirection(/*orientation*/ ctx[3])}`);
    			attr_dev(img, "alt", "rover");
    			add_location(img, file, 11, 2, 296);
    			attr_dev(div, "class", "absolute top-0");
    			add_location(div, file, 10, 0, 265);
    		},
    		...
    		p: function update(ctx, [dirty]) {
    			if (dirty & /*orientation*/ 8 && img_style_value !== (img_style_value = `transform: rotate(${/*orientation*/ ctx[3]}deg)`)) {
    				attr_dev(img, "style", img_style_value);
    			}

    			if (dirty & /*title, row, col, orientation*/ 15 && img_title_value !== (img_title_value = `${/*title*/ ctx[0]}-${/*row*/ ctx[1]}-${/*col*/ ctx[2]}-${getRoverDirection(/*orientation*/ ctx[3])}`)) {
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	const writable_props = ["title", "row", "col", "orientation"];
    	...
    	return [title, row, col, orientation];
    }

    class Rover extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init(this, options, instance, create_fragment, safe_not_equal, { title: 0, row: 1, col: 2, orientation: 3 });
    		...
    		if (/*orientation*/ ctx[3] === undefined && !("orientation" in props)) {
    			console.warn("<Rover> was created without expected prop 'orientation'");
    		}
    	}
    ...
    function create_if_block(ctx) {
    	...
    	rover = new Rover({
    			props: {
    				...
    				orientation: /*Item*/ ctx[6].rover.orientation,
    				row: /*row*/ ctx[8],
    				col: /*col*/ ctx[5]
    			},
    			$$inline: true
    	...
    	const block = {
    		...
    		p: function update(ctx, dirty) {
    			...
    			if (dirty & /*grid*/ 1) rover_changes.orientation = /*Item*/ ctx[6].rover.orientation;
                        ...
                        if (tmpRover.name === name) {
                            ...
                            commandsArray.reduce((acc, currCommand) => {
                                if (currCommand === 'l' || currCommand === 'r') {
                                    ...
                                    const { width, height } = get_store_value(gridDimensions);
                                    const { newX, newY } = moveRoverOneStep(getRoverDirection(acc.orientation), width, height, acc.x, acc.y);
                                    acc.x = newX;
                                    acc.y = newY;
                                }
                                return acc;
    ...
    function create_fragment$2(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div2, file$2, 63, 4, 1756);
    			...
    			add_location(button, file$2, 82, 4, 2441);
    			attr_dev(div3, "class", "flex flex-row");
    			add_location(div3, file$2, 62, 2, 1724);
    			add_location(form, file$2, 61, 0, 1673);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    function instance$2($$self, $$props, $$invalidate) {
    	...
    	const handleSubmit = e => {
    		...
    		if (matchedPhase) {
    			const coords = upperCaseCommads.replace(matchedPhase.qualifier, "").trim();
